...
describe('app', () => {
    ...
    //RemoveUser Error
    it('should return UserWithOpenRentError', async() => {
        const app = new App(userRepo,bikeRepo,rentRepo)
        const User101 = new User('Joao','j@unifesp.br','1234')
        await app.registerUser(User101)
        ...
        await expect( await app.removeUser('j@unifesp.br') ).rejects.toThrow(UserWithOpenRentError)
    })
    ...
    it('should sucessfully remove a user', async() => {
        ...
        const User101 = new User('Joao','j@unifesp.br','1234')
        await app.registerUser(User101)
        await app.removeUser('j@unifesp.br')
        //errado, mas deu certo
        // o correto seria user findUser e esperar UserNotFoundError
        const userList = app.userRepo.list()

        expect(userList[0]).toEqual(undefined)
    })
